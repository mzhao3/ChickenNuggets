    ...
    if (cursorValue >= 5 && cursorValue < 8) {  //Food that can't be cooked nor juice
      ...
      for (int i = 0; i < menu.length; i++) {
        for (int j = 0; j < menu[i].components.size(); j++) { 
          ...
          //if there nothing is prepared, break
